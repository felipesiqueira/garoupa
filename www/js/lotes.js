	...
	db = window.sqlitePlugin.openDatabase({name: 'database.db', location: 'default'});	
		...
		db.transaction(function (tx) 
		{
			...
			tx.executeSql(query, [], function (tx, resultSet) 
			{
				...
				db.transaction(function (tx) 
				{
					var query = "SELECT * FROM t_lotes WHERE id_fazenda=?";
					...
					{ 
						...
						for(var x = 0; x < resultSet.rows.length; x++) 
						{											
							var dados = [resultSet.rows.item(x).lote, '<a href="editLote.html?id_lote='+resultSet.rows.item(x).id_lote+'"><img src="img/edit.png" class="iconeTable"></a>'];		
				...
				db.transaction(function (tx) 
				{
					var query = "SELECT * FROM t_lotes WHERE id_fazenda=? AND id_lote=?";

					tx.executeSql(query, [id_fazenda, id_lote], function (tx, resultSet) 
					{ 
				...
				db.transaction(function (tx) 
				{
					...
					{ 
						...
						{	
							...
							//$("#animaisED select").val(resultSet.rows.item(x).id_animal);
						}
					});			
				});
			...
			db.transaction(function(tx, results) 
			{
				...
				{ 
					...
					{ 
						...
						tx.executeSql('INSERT INTO t_lotes VALUES (?,?,?,?,?,?,?)', [null, nome, null, hash, user, sync, id_fazenda]);
						...
						{ 
							...
							for(i=0; i < selO.length; i++)
							{
								...
								{
									tx.executeSql('INSERT INTO t_lotesxanimal VALUES (?,?,?,?,?,?)', [insertID, selO.options[i].value, hash, user, sync, null]);	
								}
							}
						});	
					});
			...
			db.transaction(function(tx, results) 
			{
				...
				{ 
					...
					for(i=0; i < selO.length; i++)
					{
						if(selO.options[i].selected)
						{
							tx.executeSql('INSERT INTO t_lotesxanimal VALUES (?,?,?,?,?,?)', [id, selO.options[i].value, hash, user, sync, null]);	
						}
					}
